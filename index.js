...
app.get("/jokes/:id", (req, res) =>{
    const jokId = parseInt(req.params.id);
    const jokeFunde = jokes.find((j) => j.id === jokId);
    const jokeIndex = jokes.findIndex((j) => j.id === jokId);
    if(jokeIndex <= jokes.length && jokeIndex >= 0){
      res.json(jokeFunde);  
      console.log("found it")

    }else{
      console.error({error:"There are no Elements"})
      res.json({error:"There are no Elements"})
    }
...
app.get("/filter/", (req, res) =>{
  ...
  const jokeFunde = jokes.filter((j) => j.jokeType === type );
  if (jokeFunde.length !== 0) {
    res.json(jokeFunde);
    console.log("found it")
  } else {
    console.error({error:"There are no Elements"})
...
app.patch("/jokes/:id", (req, res)=>{
  const jokId = parseInt(req.params.id);
  const exisctedJoke = jokes.find((j)=> j.id === jokId)
  const PatchJoke = {
    id:jokId,
    jokeType: req.body.type||exisctedJoke.jokeType,
    jokeText:req.body.text || exisctedJoke.jokeText,
  }
  const jokeIndex = jokes.findIndex((j) => j.id === jokId);
 if (jokeIndex <= jokes.length && jokeIndex >= 0) {
...
app.delete("/jokes/deleteAll", (req, res) =>{
  ...
  if(jokesSize > 0){
    ...
    console.error({error:"you don't have any Joke"})
    res.json({error:"you don't have any Joke"})
  }
